...
public class BooksApiController {
    ...
    public Result booksList(@PathVariable("uid") String id) {
        ...
        try {
            ...
            for(int i=0;i<collect.size();i++) {
                // parsing할 url 지정(API 키 포함해서)
                String url = "https://www.aladin.co.kr/ttb/api/ItemLookUp.aspx?ttbkey=ttbhyejmh1853001&itemIdType=ISBN13&SearchTarget=Book&ItemId=" + collect.get(i).getIsbn() + "&output=xml&Version=20131101&Cover=Big";
    ...
    public Result booksSearch(@PathVariable("uid") String uid, @PathVariable("searchOne") String searchOne) {
        ...
        try {
            // parsing할 url 지정(API 키 포함해서)
            String url = "http://www.aladin.co.kr/ttb/api/ItemSearch.aspx?ttbkey=ttbhyejmh1853001&Sort=SalesPoint&Query=" + searchOne + "&QueryType=Keyword&start=1&SearchTarget=Book&output=xml&Version=20131101&Cover=Big";
